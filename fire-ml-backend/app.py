    ...
    def train_analyzer(self, incidents_data):
        ...
        try:
            ...
            X_clf_selected, selected_clf_features = self.select_important_features(
                X_clf, y_clf, self.classification_features, top_k=5
            )
            self.classification_features = selected_clf_features
      
            # SIMPLIFIED CLASSIFIER - Reduced complexity
...
def performance_trends():
    ...
    try:
        if len(fire_incidents) < 10: # Increased minimum from 5 to 10
            return jsonify({
                'error': 'Need at least 10 incidents for reliable training',
...
def get_training_feedback():
    ...
    try:
        ...
        return jsonify({'error': str(e)}), 500
@app.route('/api/comprehensive-feedback', methods=['POST'])
